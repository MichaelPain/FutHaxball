// test/runAdditionalTests.js - Script per eseguire i test delle funzionalità aggiuntive
...
console.log('Esecuzione dei test per le funzionalità aggiuntive di HaxBall Clone...');
console.log(`Trovati ${additionalTestFiles.length} file di test`);
...
function runTests() {
  ...
  additionalTestFiles.forEach(testFile => {
    ...
    exec(`npx mocha ${fullPath}`, (error, stdout, stderr) => {
      ...
      if (completedTests === additionalTestFiles.length) {
        console.log('\n=== Riepilogo dei Test delle Funzionalità Aggiuntive ===');
        console.log(`Test completati: ${completedTests}/${additionalTestFiles.length}`);
        console.log(`Test falliti: ${failedTests}`);
        
        if (failedTests === 0) {
          console.log('\nTutti i test delle funzionalità aggiuntive sono stati completati con successo!');
        } else {
